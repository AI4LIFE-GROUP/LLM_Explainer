...
class LimeTabularExplainer(object):
    ...
    def explain_instance(self,
        ...
        # COMMENT FROM DL:
        # Swapping out __data_inverse for self.__normal_perturbation
        if perturbations is not None:
            data = perturbations[0]
            yss = perturbations[1]
        else:
            data = self.__normal_perturbation(data_row, num_samples, seed)
    ...
    def __normal_perturbation(self, data_row, num_samples, seed=None):
        """Generates a neighborhood around a prediction.
        
        Use NormalPerturbation from openxai.explainers.catalog.perturbation_methods
        """
        # Set parameters
        flip = np.sqrt(2/np.pi)*self.std
        feature_types = ['d' if (i in self.categorical_features) else 'c'\
                         for i in range(data_row.shape[-1])]
        feature_mask = np.zeros(len(feature_types), dtype=bool)

        # Construct perturbations
        perturbation = NormalPerturbation("tabular", mean=0, std_dev=self.std,
                                          flip_percentage=flip, seed=seed)
        data = perturbation.get_perturbed_inputs(data_row, feature_mask,
                                                 num_samples, feature_types)
